...
The complete catalog of what a device can expose, grouped by family. `<p>` stands for the device prefix (`<name><name_suffix>`, e.g. `washing_machine_em`); `<cycle>` for each configured cycle (`daily`, `monthly`, …). Which families a device gets is decided by its options — see [Configuration](configuration.md).

Each entity carries two markers:

- **Recorder class** — how to treat it in the recorder (details in [Recorder Setup](recorder.md)):
  - 🚫 *never record* — a Lean cycle meter that writes its own long-term statistics; recording it corrupts the series
...
- **↺ resettable** — supports the `energy_insights_monitor.reset` entity service (zeroes the value). The 🚫 cycle meters are native Lean entities instead, maintained via Lean's own services — see [Services & Actions](services.md).
...
| `sensor.<p>_energy_<cycle>` 🚫 | kWh | Lean cycle meter over the lifetime: live counter in the UI, one consolidated LTS row per closed cycle |
...
| `sensor.<p>_energy_cost_<cycle>` 🚫 | € | Lean cycle meter over the cost integrator |
...
| `sensor.<p>_energy_from_self_<cycle>` 🚫 | kWh | Lean cycle meters over the two splits |
| `sensor.<p>_energy_from_grid_<cycle>` 🚫 | kWh | ” |
| `sensor.<p>_energy_self_sufficiency_lifetime` 💤 | % | Live all-time ratio `from_self / total` |
| `sensor.<p>_energy_self_sufficiency_<cycle>` 🚫 | % | Lean **gauge** meters: the percentage is snapshotted into one LTS row per cycle (it can move up and down within the period) |
...
| `sensor.<p>_energy_from_grid_savings_<cycle>` 🚫 | € | Lean cycle meters over the two |
| `sensor.<p>_energy_from_grid_cost_<cycle>` 🚫 | € | ” |

## Cycles — requires the `run:` block

### Run detection and cycle boundary

| Entity | Unit | Description |
| --- | --- | --- |
| `binary_sensor.<p>_running` 📈 | — | The gatekeeper: `on` while the appliance runs, according to the configured power threshold or template trigger. Cycle open/close and standby gating all follow this |
...
| `sensor.<p>_cycle_validation_status` 📈 | — | Verdict on the last closed cycle: `valid`, `too_short`, `too_long`, `too_little_energy`, `too_much_energy` (see [limits](configuration.md#cycle-limits-limits)) |
...
| `sensor.<p>_cycles_count_<cycle>` 🚫 | cycles | Lean cycle meters: how many runs / how much running time per period |
| `sensor.<p>_cycles_duration_<cycle>` 🚫 | s | ” |
